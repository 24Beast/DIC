...
def create_human_list(human_pkl_path):
    ...
    return [
        {"img_id": entry["img_id"], "caption_list": entry["caption_list"]}
        for entry in human_captions
    ]
...
def contextual_substitute(
    ...
    for entry in human_list:
            ...
            for human_caption in entry["caption_list"]:
                ...
                for i, token in enumerate(tokens):
                    if should_ignore_token(token) and i == len(tokens) - 1:
                        ...
                        substituted_token = substitute_with_glove(
                            token, machine_corpus, glove_model, similarity_threshold
                        )
        ...
        modified_captions_grouped.append(
            {"img_id": img_id, "modified_caption_list": modified_captions_for_img}
        )
...
def save_modified_captions_to_file(
    modified_human_captions_grouped, output_folder, output_file
):
...
def process_corpora_by_img_id(
    ...
    if mode == "contextual":
        modified_human_captions_grouped = contextual_substitute(
            human_list,
            machine_dict,
            machine_corpus,
            glove_model,
        ...
        modified_human_captions_grouped = constant_substitute(
            human_list, machine_dict, machine_corpus
        )
...
def get_parser():
    ...
    parser.add_argument(
        "--mode",
        default="contextual",
        choices=["contextual", "constant"],
        help='Choose substitution mode: "contextual" or "constant".',
    )
    parser.add_argument(
        "--output_folder",
        default="output",
        type=str,
        help="Folder to save modified human captions",
    )
    parser.add_argument(
        "--output_file",
        default="modified_human_captions.txt",
        type=str,
        help="File to save modified human captions",
    )
    return parser
...
def main(args):
    ...
    save_modified_captions_to_file(
        modified_human_list_grouped, args.output_folder, args.output_file
    )
