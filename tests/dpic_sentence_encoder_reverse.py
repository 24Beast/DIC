...
def calculate_dpic(
    data_obj, dpic_model, mode="non-contextual", threshold=0.5
):
    print(
...
def main():
    ...
    parser.add_argument(
        "--embed_model", default = "all-MiniLM-L6-v2", help="sentence embedding model to be used."
    )
    parser.add_argument(
        "--seed",
        default=0,
    ...
    if args.mode == "non-contextual":
        non_contextual_dpic = calculate_dpic(
            data_obj, dpic_model, mode="non-contextual"
        )
        results.append(
            {
                "mode": "non-contextual",
        ...
        for threshold in contextual_thresholds:
            contextual_dpic = calculate_dpic(
                data_obj, dpic_model, mode="contextual", threshold=threshold
            )
            results.append(
                {
                    "mode": "contextual",
